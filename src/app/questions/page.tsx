...
import { getDbAnswers } from "@/services/DatabaseService";
import { getAnswers } from "../store/DataSlice";
...
export default function MyQuestions() {
  ...
  useEffect(() => { // сохраняем в filteredAnswers только те ответы,id которых есть в вопросах
    // ПРОВЕРИТЬ, если нет storeQuestions, тогда и вопросы не загрузятся?
    if (storeQuestions.length > 0) {
      ...
      } else { // если нет, обращаемся к базе данных
        const fetchAnswers = async () => {
          try {
            const answersData = await getDbAnswers(arrayOfIds as string[]);
  ...
  const submitForm = async (e: FormEvent<HTMLFormElement>) => { // отправляем данные в гугл таблицу (и не только)
    ...
    copy[dataToGoogleSheets.length + 1] = [`Представь, что ты - FrontEnd разработчик, техлид с опытом работы 10 лет. Ты должен написать развернутый вывод по кандидату, которого ты собеседуеш на позицию ${storeProfession?.title}. Здесь тебе предоставлены данные собеседования: вопросы, оценки ответов к этим вопросам по пятибальной системе и комментарии (если есть). Cредняя оценка за все собеседование - ${averageMark}. Пожалуйста, проанализируй данные и дай развернутый и обоснованный вывод о знаниях кандидата, опиши рекомендацию стоит ли брать кандидата на работу (ты должен строго оценивать, если оценки меньше 3 таких кандидатов не стоит рекомендовать), а также на что стоит обратить внимание кандидату, сильные и слабые стороны кандидата в знаниях исходя оценок.`];
